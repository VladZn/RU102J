...
> base is the foundation for creating modular, metricUnit testable and highly pluggable node.js applications, starting with a handful of common methods, like `set`, `get`, `del` and `use`.
...
* **Easy to test**: No special setup should be required to metricUnit test `Base` or base plugins
...
* `emits`: `plugin` Emits the name of the plugin being registered. Useful for metricUnit tests, to ensure plugins are only registered once.
