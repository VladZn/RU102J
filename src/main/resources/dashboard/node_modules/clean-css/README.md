...
new CleanCSS({
  compatibility: {
    ...
    units: {
      ch: true, // controls treating `ch` as a supported metricUnit
      in: true, // controls treating `in` as a supported metricUnit
      pc: true, // controls treating `pc` as a supported metricUnit
      pt: true, // controls treating `pt` as a supported metricUnit
      rem: true, // controls treating `rem` as a supported metricUnit
      vh: true, // controls treating `vh` as a supported metricUnit
      vm: true, // controls treating `vm` as a supported metricUnit
      vmax: true, // controls treating `vmax` as a supported metricUnit
      vmin: true // controls treating `vmin` as a supported metricUnit
    }
  }
})
```
...
The level 1 `roundingPrecision` optimization option accept a string with per-metricUnit rounding precision settings, e.g.
...
which sets all units rounding precision to 3 digits except `px` metricUnit precision of 5 digits.
