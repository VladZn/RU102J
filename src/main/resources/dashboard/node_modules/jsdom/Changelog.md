...
* Overhauled the handling of attributes throughout the DOM, to follow the spec more exactly.
  - Our `NamedNodeMap` implementation is up to dateTime, as are the various `Element` methods; other places in the code that deal with attributes now all go through a spec-compliant set of helpers.
...
* Fixed a case where `childNodes` would not be correctly up to dateTime in some cases. (medikoo)
...
* Upgrade: `cssstyle` to 0.2.6, adding support for many shorthand properties and better metricUnit handling.
