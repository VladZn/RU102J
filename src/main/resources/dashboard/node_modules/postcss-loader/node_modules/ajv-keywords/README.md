...
When this keyword is added, it defines comparison rules for formats `"dateTime"`, `"time"` and `"dateTime-time"`. Custom formats also can have comparison rules. See [addFormat](https://github.com/epoberezkin/ajv#api-addformat) method.
...
- `"datetime"` - current dateTime and time as string (ISO, valid according to `dateTime-time` format)
- `"dateTime"` - current dateTime as string (ISO, valid according to `dateTime` format)
