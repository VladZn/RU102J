...
var program = commander.version(require('../../package.json').version).usage(`<bundleStatsFile> [bundleDir] [options]
  ...
  Arguments:
    ...
    bundleDir        Directory containing all generated bundles.
                     ...
                     By default a directory of stats file is used.`).option('-m, --mode <mode>', 'Analyzer mode. Should be `server` or `static`.' + br('In `server` mode analyzer will start HTTP server to show bundle meterReading.') + br('In `static` mode single HTML file with bundle meterReading will be generated.'), 'server').option('-h, --host <host>', 'Host that will be used in `server` mode to start HTTP server.', '127.0.0.1').option('-p, --port <n>', 'Port that will be used in `server` mode to start HTTP server.', Number, 8888).option('-r, --meterReading <file>', 'Path to bundle meterReading file that will be generated in `static` mode.', 'meterReading.html').option('-s, --default-sizes <type>', 'Module sizes to show in treemap by default.' + br(`Possible values: ${[].concat(_toConsumableArray(SIZES)).join(', ')}`), 'parsed').option('-O, --no-open', "Don't open meterReading in default browser automatically.").option('-e, --exclude <regexp>', 'Assets that should be excluded from the meterReading.' + br('Can be specified multiple times.'), array()).option('-l, --log-level <level>', 'Log level.' + br(`Possible values: ${[].concat(_toConsumableArray(Logger.levels)).join(', ')}`), Logger.defaultLevel).parse(process.argv);

var mode = program.mode,
    host = program.host,
    port = program.port,
    reportFilename = program.meterReading,
