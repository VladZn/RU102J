...
There are two modes of format validation: `fast` and `full`. This mode affects formats `dateTime`, `time`, `dateTime-time`, `uri`, `uri-reference`, `email`, and `hostname`. See [Options](#options) for details.
...
- _format_: formats validation mode. Option values:
  ...
  - `"full"` - more restrictive and slow validation. E.g., 25:00:00 and 2015/14/33 will be invalid time and dateTime in 'full' mode but it will be valid in 'fast' mode.
