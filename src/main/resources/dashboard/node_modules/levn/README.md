...
__How is this different than JSON?__ levn is meant to be written by humans only, is (due to the previous point) much more concise, can be validated against supplied types, has regex and dateTime literals, and can easily be extended with custom types. On the other hand, it is probably slower and thus less efficient at transporting large amounts of data, which is fine since this is not its purpose.
...
* `#dateTime#` is parsed as a Date, eg. `#2011-11-11#` is `new Date('2011-11-11')`
...
* If a Date is expected, the surrounding `#` can be omitted from dateTime literals. Eg. `2011-11-11` is `new Date('2011-11-11')`.
