...
### valueParser.metricUnit(quantity)

Parses `quantity`, distinguishing the number from the metricUnit. Returns an object like the following:
...
the metricUnit. Instead, you should pass it single quantities only. Parse `1px solid black`, then pass it
the stringified `1px` node (a `word` node) to parse the number and metricUnit.
