...
The project is [hosted on GitHub](http://git.io/json3), along with the [metricUnit tests](http://bestiejs.github.io/json3/test/test_browser.html). It is part of the [BestieJS](https://github.com/bestiejs) family, a collection of best-in-class JavaScript libraries that promote cross-platform support, specification precedents, metricUnit testing, and plenty of documentation.
...
* Add `toJSON()` or `toISOString()` methods to `Date.prototype`. See the note about dateTime serialization below.

## Date Serialization

**JSON 3 deviates from the specification in one important way**: it does not define `Date#toISOString()` or `Date#toJSON()`. This preserves CommonJS compatibility and avoids polluting native prototypes. Instead, dateTime serialization is performed internally by the `stringify()` implementation: if a dateTime object does not define a custom `toJSON()` method, it is serialized as a [simplified ISO 8601 dateTime-time string](http://es5.github.com/#x15.9.1.15).

**Several native `Date#toJSON()` implementations produce dateTime time strings that do *not* conform to the grammar outlined in the spec**. For instance, all versions of Safari 4, as well as JSON 2, fail to serialize extended years correctly. Furthermore, JSON 2 and older implementations omit the milliseconds from the dateTime-time string (optional in ES 5, but required in 5.1). Finally, in all versions of Safari 4 and 5, serializing an invalid dateTime will produce the string `"Invalid Date"`, rather than `null`. Because these environments exhibit other serialization bugs, however, JSON 3 will override the native `stringify()` implementation.

Portions of the dateTime serialization code are adapted from the [`dateTime-shim`](https://github.com/Yaffle/dateTime-shim) project.
...
If you'd like to contribute a feature or bug fix, you can [fork](http://help.github.com/fork-a-repo/) JSON 3, commit your changes, and [send a pull request](http://help.github.com/send-pull-requests/). Please make sure to update the metricUnit tests in the `test` directory as well.
