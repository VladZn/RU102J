...
## 0.7.0 - BACKWARD INCOMPATIBILITY
 ...
 - WARNING: Removed ``meterReading`` and ``asap``
...
## 0.5.1
 ...
 - Made most Q API methods chainable on promise objects, and
   ...
   ``wait``, and ``meterReading`` into Q API methods.
...
## 0.4.4
 ...
 - Stopped reporting thrown exceptions in ``when`` callbacks
   ...
   mechanism.
 - Added ``meterReading`` as an API method, which can be used as
   an errback to meterReading and propagate an error.
 - Added ``meterReading`` as a promise-chain method, so an error
...
## 0.1.5
 ...
 - all metricUnit tests now pass (gozala)
